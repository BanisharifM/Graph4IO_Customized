...
df = pd.read_csv(fp_sample_csv, delimiter=',', skiprows=1)
dataset = df.values
...
Y = dataset[:, n_dims - 1]
Y_numeric = np.where(np.core.defchararray.isnumeric(Y.astype(str)), Y, 0)
Y_numeric = Y_numeric.astype(float)

print("max(Y) =", max(Y_numeric), ", min(Y) =", min(Y_numeric))
...
X_train_sparse, X_test_sparse, y_train, y_test = train_test_split(X_sparse, Y, random_state=1, shuffle=False)
print("X_train.type=", type(X_train_sparse))
print("X_train.shape=", X_train_sparse.shape)
list_x_train = X_train_sparse.toarray().tolist()
list_x_test = X_test_sparse.toarray().tolist()
